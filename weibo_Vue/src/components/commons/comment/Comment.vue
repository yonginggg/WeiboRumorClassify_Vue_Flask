    ...
    <div class="comments-list">
      <div class="item animated slideInRight" v-for="(item,index) in comments" v-bind:key="index">
        <div class="item-heading">
          <div class="left">
            <img :src="item.AvatarUrl" @click="openGithub(item.GithubUrl,item.Pensonal,item.Username)" />
            <span class="item-username" @click="openGithub(item.GithubUrl,item.Pensonal,item.Username)">{{item.Username}}</span>
          </div>
          <div class="right">
            <div class="date">{{dateToStr(new Date(item.CreatedAt))}}</div>
          </div>
        </div>
...
export default {
  ...
  methods: {
    ...
    openGithub(url,userId,userName) {
      // window.open(url);
